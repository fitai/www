...
function drawSmallTickMarks(options) {
	...
	var tickvalue = options.levelRadius - 0,
	    iTick = 0,
...
function drawLargeTickMarks(options) {
	...
	var tickvalue = options.levelRadius - 10,
	    iTick = 0,
        gaugeOptions = options.gaugeOptions,
        iTickRad = 0,
        innerTickY,
	...
	tickvalue = options.levelRadius - 10;
...
function drawTextMarkers(options) {
	...
	for (iTick = 18; iTick < 180; iTick += 18) {
		...
		if (iTick <= 18) {
			...
			options.ctx.fillText(iTickToPrint, (options.center.X - gaugeOptions.radius - 12) + innerTickX + 9,
					(gaugeOptions.center.Y - gaugeOptions.radius - 12) + innerTickY - 3);
		} else if (iTick < 145) {
...
function convertSpeedToAngle(options) {
	...
	* equivelant angle.
	*/
	var iSpeed = (options.speed / 1),
	    iSpeedAsAngle = (iSpeed * 18);
